...
import { getPayout, retryPayout } from "@/lib/store";
import { isSameOrigin, rateLimit } from "@/lib/security";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** finance_admin retries a failed payout (idempotent engine + reconciliation). */
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const rl = rateLimit(`payout-retry:${session.sub}`, 30, 60_000);
  if (!rl.ok) return NextResponse.json({ ok: false, error: "Too many attempts, slow down." }, { status: 429 });

  const result = await retryPayout(params.id, { userId: session.sub, name: session.name, role: "finance_admin" });
